...
object Example41 {
  private def processBox[T: TypeTag](box: Box[T]): Unit = {
    ...
    box match {
      ...
      case Box(content: Seq[Seq[Int]]) if t =:= typeOf[Seq[Seq[Int]]]       => println("Content is Seq[Seq[Int]]")
      case Box(content: Seq[Seq[String]]) if t =:= typeOf[Seq[Seq[String]]] => println("Content is Seq[Seq[String]]")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val seqSeqIntBox    = Box(Seq(Seq(1, 2, 3), Seq(4, 5, 6)))
    val seqSeqStringBox = Box(Seq(Seq("a", "b", "c"), Seq("d", "e", "f")))

    processBox(intBox)
    processBox(stringBox)
    processBox(seqIntBox)
    processBox(seqStringBox)
    processBox(seqSeqIntBox)
    processBox(seqSeqStringBox)
  }
}
